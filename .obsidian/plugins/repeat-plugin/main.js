...
function getNotesDue(dv, ignoreFolderPath, ignoreFilePath, enqueueNonRepeatingNotes, defaultRepeat) {
  const now2 = DateTime.now();
...
function getNextDueNote(dv, ignoreFolderPath, ignoreFilePath, enqueueNonRepeatingNotes, defaultRepeat) {
  var _a;
  const page = (_a = getNotesDue(dv, ignoreFolderPath, ignoreFilePath, enqueueNonRepeatingNotes, defaultRepeat)) == null ? void 0 : _a.first();
  if (!page) {
    return;
  }
  return page;
...
var RepeatView = class extends import_obsidian3.ItemView {
  ...
  async setPage(ignoreFilePath) {
    ...
    const page = getNextDueNote(this.dv, this.settings.ignoreFolderPath, ignoreFilePath, this.settings.enqueueNonRepeatingNotes, this.settings.defaultRepeat);
    if (!page) {
...
var RepeatNoteSetupModal = class extends import_obsidian4.Modal {
  ...
  onOpen() {
    ...
    weekdays2.forEach(({ key, label }) => {
      new import_obsidian4.Setting(weekdayContainerEl).setName(label).addToggle((toggle) => {
        var _a;
...
var RepeatPluginSettingTab = class extends import_obsidian5.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian5.Setting(containerEl).setName("Default `repeat` property").setDesc(`Used to populate "Repeat this note..." command's modal. Ignored if the supplied value is not parsable.`).addText((component) => {
